# Axon Dashboard Guide

The Axon React dashboard is a purpose-built, self-hosted UI for teams operating AI inference
workloads in production. It ships as part of the Axon Phase 4 platform.

---

## Dashboard vs Grafana

Axon includes two observability UIs with different purposes:

| | Axon Dashboard | Grafana |
|---|---|---|
...
| **Data source** | Axon backend REST API (`/v1/attribution`, `/v1/spans`, `/v1/budgets`) | Prometheus, Loki, Postgres (direct) |
| **Auth** | Axon API key (`X-Axon-API-Key`) | Grafana native auth |
...
health). Use the Axon dashboard for business-level questions: which features cost the most,
...
The dashboard is available at **http://localhost:5173** once the `axon-dashboard` container is
healthy. It depends on `axon-backend` being healthy first; Docker Compose enforces this ordering.
...
```bash
VITE_AXON_BACKEND_URL=https://axon.internal.example.com \
VITE_AXON_API_KEY=axon_live_your_key_here \
npm run dev
```
...
docker build -t axon-dashboard .
docker run -p 5173:80 axon-dashboard
```
...
**What it shows:** A high-level breakdown of all inference spend attributed through the Axon SDK.
...
**What it shows:** Decisions made by the Axon adaptive model router and their cost impact.
...
**Expected data:** Populated only when the SDK router is active (`axon.patch(client, router=True)`)
...
|---|---|---|
| `VITE_AXON_BACKEND_URL` | `http://localhost:8000` | Base URL of the Axon backend API. Set to your backend's public or internal URL in production. |
| `VITE_AXON_API_KEY` | _(none)_ | API key sent as `X-Axon-API-Key` on every request. Must have at minimum `viewer` role to read data. Use an `engineer` or `admin` key to manage budgets. |
...
The `axon-dashboard` service in `docker-compose.yml` picks these up automatically.
